...
# Generate CA key
openssl req -new -x509 -keyout snakeoil-ca-1.key -out snakeoil-ca-1.crt -days 1825 -subj '/CN=ap-southeast-1.compute.internal/OU=NONPROD/O=HomeCredit_VN/L=PaloAlto/S=Ca/C=VN' -passin pass:confluent -passout pass:confluent
...
do
	...
	keytool -genkey -noprompt \
				 -alias $i \
				 -dname "CN=$i,OU=NONPROD,O=HomeCredit_VN,L=PaloAlto,S=Ca,C=VN" \
                                 -ext san=dns:$i \
	...
	# Save creds
  	echo "kafka" > ${i}_sslkey_creds
  	echo "kafka" > ${i}_keystore_creds
  	echo "kafka" > ${i}_truststore_creds
